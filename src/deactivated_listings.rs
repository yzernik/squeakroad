...
impl Context {
    pub async fn raw(
        ...
        let listing_cards = ListingCardDisplay::all_deactivated(&mut db, PAGE_SIZE, page_num)
            .await
...
pub fn deactivated_listings_stage() -> AdHoc {
    AdHoc::on_ignite("Deactivated Listings Stage", |rocket| async {
        rocket.mount("/deactivated_listings", routes![index])
    })
}
